...
-- Constraints for table `ospos_devices`
ALTER TABLE `ospos_devices`
  ADD CONSTRAINT `ospos_devices_ibfk_1` FOREIGN KEY (`modified_by_id`) REFERENCES `ospos_employees` (`person_id`);
...
CREATE TABLE IF NOT EXISTS `ospos_tickets` (
  ...
  `repair_duration` INT(10) DEFAULT 0,    -- duration in minutes
...
ALTER TABLE `ospos_tickets`
  ...
  ADD CONSTRAINT `ospos_tickets_ibfk_5` FOREIGN KEY (`modified_by_id`) REFERENCES `ospos_employees` (`person_id`);
